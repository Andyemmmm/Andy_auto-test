...
@ddt.ddt
class Test_iwask_doctorinfo_click(unittest.TestCase, Iw_data_collect):
    def setUp(self):
        self.logger = logger()
    ...
    def test_click(self, num):
        ...
        url = f"{Page_Iwask_doctorinfo.url}channel=test_zrp{channel}"
        self.logger.info(f'配置了channel后的URL为：{url}')
        self.driver.get(url)

        self.click_event_back(Page_Iwask_doctorinfo.lco_图文咨询按钮, num)
        self.click_event_back(Page_Iwask_doctorinfo.lco_去咨询, num)
