...
@ddt.ddt
class Test_iwask_operationtopic_click(unittest.TestCase, Iw_data_collect):
    def setUp(self):
        self.logger = logger()
    ...
    def test_click(self, num):
        ...
        url = f"{Page_Iwask_operationtopic.url}channel=test_zrp{channel}"
        self.logger.info(f'配置了channel后的URL为：{url}')
        self.driver.get(url)

        self.driver.execute_script('window.scrollTo(0, document.body.scrollHeight);')
        time.sleep(1)
        self.click_event_back(Page_Iwask_operationtopic.loc_第一个推荐医生卡片, num)
        self.click_event_back(Page_Iwask_operationtopic.loc_第一个推荐医生_去咨询, num)
